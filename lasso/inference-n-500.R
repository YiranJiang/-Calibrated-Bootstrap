...
pen <- pen.seq[48] ## cross-validated results
...
for (nn in 1:NN){
  ...
  out.1 <- boot.ra (xy, beta, this.sigma_sq,sigma_sq, res.ML, sir=FALSE, m=as.integer(n), M=c(this.round,this.N),
                      level.target=level.target, pen = pen, sample_pen = 2*pen, upper_scale = 5, lower_scale= 0.75, c = 5000, decay = 1)
  ...
  save.image(file = paste0(save.directory,"/result-",this.id,"-",nn,"-n_500-lasso-dr-out.RDS"))
...
saveRDS(this.cover.mat, paste0(save.directory,"/result-",this.id,"-n_500-lasso.RDS"))
saveRDS(this.length.vec, paste0(save.directory,"/length-",this.id,"-n_500-lasso.RDS"))