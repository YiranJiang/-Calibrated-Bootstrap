...
for (nn in 1:NN){
  ...
  level.target <- c(0.10,0.5,0.90)

  this.N <- 100
  this.round <- 100

  out.1 <- boot.ra (xy, beta, this.sigma_sq,sigma_sq, res.ML, sir=FALSE, m=as.integer(n), M=c(this.round,this.N),
                      level.target=level.target, pen = pen, sample_pen = 2*pen, upper_scale = 5, lower_scale= 0.75, c = 1000, decay = 1)
  ...
  save.image(file = paste0(save.directory,"/result-",this.id,"-",nn,"-n_100-lasso-dr-out.RDS"))
  ...
  for(i in 1:length(level.seq)){
    this.length.vec[1,i] <- this.length.vec[1,i] + quantile(cal.vec, level.seq[i])
...
saveRDS(this.cover.mat, paste0(save.directory,"/result-",this.id,"-n_100-lasso.RDS"))
saveRDS(this.length.vec, paste0(save.directory,"/length-",this.id,"-n_100-lasso.RDS"))